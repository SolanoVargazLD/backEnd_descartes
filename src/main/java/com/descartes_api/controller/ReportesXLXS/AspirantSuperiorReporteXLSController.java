...
public class AspirantSuperiorReporteXLSController {
    @Autowired
    ...
    //Constructor
    public AspirantSuperiorReporteXLSController(){
        this.workbook= new XSSFWorkbook();  //Instaciar el archivo para el Excel
    }

    @GetMapping("/ReporteLicenciatureXlxs")
    public void getAspirantBasicPreescolarXLSX(HttpServletResponse response,  @RequestParam("nameCareer") String nameCareer)throws IOException {

        response.setContentType("application/octet-stream");
        long time = System.currentTimeMillis();
        String headerKey = "Content-Disposition";
        String headerValue = "attachment; filename=reporte_Licenciatua_"+nameCareer+"_" + time + ".xlsx";
        ...
        List<AspirantSuperior> datos= aspirantSuperiorService.listAspirantSuperiorPorProfecion(nameCareer);
        int rowCount = 1;
        for( AspirantSuperior dato: datos){
