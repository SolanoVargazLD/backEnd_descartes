...
public class AspirantSuperiorPosgradoReporteXLSController {
    @Autowired
    ...
    //Constructor
    public AspirantSuperiorPosgradoReporteXLSController(){
        this.workbook= new XSSFWorkbook();  //Instaciar el archivo para el Excel
    }

    @GetMapping("/ReportePosgradoXlxs")
    public void getAspirantBasicPreescolarXLSX(HttpServletResponse response,  @RequestParam("nivelCareer") String nivelCareer,@RequestParam("nameCareer") String nameCareer)throws IOException {

        response.setContentType("application/octet-stream");
        long time = System.currentTimeMillis();
        String headerKey = "Content-Disposition";
        String headerValue = "attachment; filename=reporte_"+nivelCareer+"_"+nameCareer+"_" + time + ".xlsx";
        ...
        createCell(row, 7,"Cedula Profesional", style);
        ...
        List<AspirantPostgraduate> datos= aspirantPostgraduateService.listAspirantSuperiorPorPosgrado(nivelCareer,nameCareer);
        int rowCount = 1;
        for( AspirantPostgraduate dato: datos){
            ...
            createCell(row, columnCount++, dato.getProfessionalLicense(), style);
